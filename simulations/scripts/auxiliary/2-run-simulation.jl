...
mean_estimator = SuperLearner([
    ...
    XGBoostRegressor(objective = "reg:squarederror", 
                    eta = 0.1, max_depth = 6, subsample = 0.5),
    XGBoostRegressor(objective = "reg:squarederror", 
                    eta = 0.01, max_depth = 3, subsample = 1.0),
], CV(nfolds = 5))
...
sl = SuperLearner([
    ...
    XGBoostClassifier(objective = "binary:logistic",
        num_round = 100, eta = 0.1, max_depth = 6, subsample = 1.0, lambda = 1),
    XGBoostClassifier(objective = "binary:logistic",
        num_round = 500, eta = 0.01, max_depth = 3, subsample = 0.5, lambda = 1),
    XGBoostClassifier(objective = "binary:logistic",
        num_round = 500, eta = 0.01, max_depth = 3, subsample = 1.0, lambda = 0.1)
], CV(nfolds = 5))
