...
mach_net_os = machine(mtp, ct2, shift) |> fit!
mtp_result_net_a_os = getestimate(ModifiedTreatment.estimate(mach_net_os, shift))
mtp_effect_net_a_os = get_effects_tuple(mtp_result_net_a_os, ψ0, n)

```

```{julia}
ests = [mtp_effect_net_a_sum, mtp_effect_net_a_os, mtp_effect_a, glm_effect_a]

tbl = (method = ["Induced MTP (Sum)", "Induced MTP (Trimmed)", "MTP (No Interference)", "GLM (No Interference)"], effects = map(x -> x.est, ests), upper = map(x -> x.upper, ests), lower = map(x -> x.lower, ests))
...
for a in as
        ...
        push!(est_grid_a, merge((shift = a, name = "Induced MTP (Trimmed)"), get_effects_tuple(mtp_result_net_a_os, ψ0, n)))
...
ests = [mtp_effect_net_m_sum, mtp_effect_net_m_os, mtp_effect_m, glm_effect_m]
...
for a in as
        ...
        push!(est_grid_m, merge((shift = a, name = "Induced MTP (Trimmed)"), get_effects_tuple(mtp_result_net_m_os, ψ0, n)))
end
...
Do everything again but this time use the covariate-conditional variance

```{julia}
function get_effects_tuple(mtp_result, ψ0, n)
        mtp_ψ = mtp_result.tmle.ψ .- ψ0
        ciwidth = 1.96 * sqrt(mtp_result.tmle.σ2cond / n)
        (est = mtp_ψ, lower = mtp_ψ - ciwidth, upper = mtp_ψ + ciwidth)
end
...
ashift = AdditiveShift(1.0)

mtp_result_a = getestimate(ModifiedTreatment.estimate(mach, mshift))
mtp_effect_a = get_effects_tuple(mtp_result_m, ψ0, n)

mtp_result_net_a_sum = getestimate(ModifiedTreatment.estimate(mach_net_sum, mshift))
mtp_effect_net_a_sum = get_effects_tuple(mtp_result_net_a_sum, ψ0, n)

mtp_result_net_a_os = getestimate(ModifiedTreatment.estimate(mach_net_os, mshift))
mtp_effect_net_a_os = get_effects_tuple(mtp_result_net_a_os, ψ0, n)

ests = [mtp_effect_net_a_sum, mtp_effect_net_a_os, mtp_effect_a, glm_effect_a]

tbl = (method = ["Induced MTP (Sum)", "Induced MTP (Trimmed)", "MTP (No Interference)", "GLM (No Interference)"], effects = map(x -> x.est, ests), upper = map(x -> x.upper, ests), lower = map(x -> x.lower, ests))
CSV.write(joinpath("results", "mtp_analysis_a_cond.csv"), tbl)
...
tbl = (method = ["Induced MTP (Sum)", "Induced MTP (Trimmed)", "MTP (No Interference)", "GLM (No Interference)"], effects = map(x -> x.est, ests), upper = map(x -> x.upper, ests), lower = map(x -> x.lower, ests))
CSV.write(joinpath("results", "mtp_analysis_m_cond.csv"), tbl)
```
...
CSV.write(joinpath("results", "mtp_grid_a_cond.csv"), est_grid_a)
...
CSV.write(joinpath("results", "mtp_grid_m_cond.csv"), est_grid_m)
