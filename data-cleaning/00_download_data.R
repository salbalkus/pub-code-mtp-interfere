...
setwd(here("data-cleaning", "raw_data"))
...
if(!(no2_2019_filename %in% list.files())){
  download.file(no2_2019_src, no2_2019_filename)
}

if(!(no2_2013_filename %in% list.files())){
  download.file(no2_2013_src, no2_2013_filename)
}
...
if(!(smart_loc_filename %in% list.files())){
  smart_loc_zip <- paste0(smart_loc_filename, ".zip")
  download.file(smart_loc_src, smart_loc_zip)
  unzip(smart_loc_zip, exdir = smart_loc_filename)
  file.remove(smart_loc_zip)
}

### Download National Walkability Index
#walk_src = r"{https://edg.epa.gov/EPADataCommons/public/OA/WalkabilityIndex.zip}"
#walk_filename = "WalkabilityIndex"

#if(!(walk_filename %in% list.files())){
#  walk_zip <- paste0(walk_filename, ".zip")
#  download.file(walk_src, walk_zip)
#  unzip(walk_zip, exdir = walk_filename)
#  file.remove(walk_zip)
#}
...
if(!(zev_filename %in% list.files())){
  download.file(zev_src, zev_filename)
}
